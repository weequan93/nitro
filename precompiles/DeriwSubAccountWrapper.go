...
type DeriwSubAccountPrecompile struct {
	precompile  ArbosPrecompile
	emitSuccess func(mech, bytes4, addr, []byte) error
}

func deriwSubAccountOwnerOnly(address addr, impl ArbosPrecompile, emit func(mech, bytes4, addr, []byte) error) (addr, ArbosPrecompile) {
	return address, &DeriwSubAccountPrecompile{
		precompile:  impl,
		emitSuccess: emit,
	}
}

func (wrapper *DeriwSubAccountPrecompile) Address() common.Address {
	return wrapper.precompile.Address()
}

func (wrapper *DeriwSubAccountPrecompile) Call(
	input []byte,
	...
	owners := state.SubAccount().SubAccountOwner()
...
func (wrapper *DeriwSubAccountPrecompile) Precompile() *Precompile {
	con := wrapper.precompile
	return con.Precompile()
}

func (wrapper *DeriwSubAccountPrecompile) Name() string {
