...
// DeriwGuardPrecompile is a precompile wrapper for those only allowed deriw guard may use
type DeriwGaurdPrecompile struct {
	precompile  ArbosPrecompile
	emitSuccess func(mech, bytes4, addr, []byte) error
}

func deriwGuardOnly(address addr, impl ArbosPrecompile, emit func(mech, bytes4, addr, []byte) error) (addr, ArbosPrecompile) {
	return address, &DeriwGaurdPrecompile{
		precompile:  impl,
		emitSuccess: emit,
	}
}

func (wrapper *DeriwGaurdPrecompile) Call(
	input []byte,
	precompileAddress common.Address,
	actingAsAddress common.Address,
	caller common.Address,
	...
	// if is other method
	owners := state.DeriwGaurd.PricerAddrs()
	isOwner, err := owners.IsMember(caller)
	if err != nil {
		return nil, burner.gasLeft, err
	}
...
func (wrapper *DeriwGaurdPrecompile) Precompile() *Precompile {
	con := wrapper.precompile
	return con.Precompile()
}
